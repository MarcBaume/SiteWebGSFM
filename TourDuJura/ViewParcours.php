	...
	if (($handle = fopen($pathFile, "r")) !== FALSE) 
	{
		...
		?>
			...
			<? 	 $sql = 'SELECT * FROM TourDuJura  WHERE IDParcours=\''.$ID. '\'';
				...
				{
				?>
			<tr onClick="ClickRows( event, <?php echo $ID ?>)" style="cursor: pointer;background-color: #f2f2f2;" >
				<?
				}
				else
				{?>
			<tr onClick="ClickRows( event, <?php echo $ID ?>)" style="cursor: pointer;background-color: #ffffff;"  >
				<?
				}?>
