...
//$bdd = new PDO('mysql:host=dxvv.myd.infomaniak.com;dbname=dxvv_gsfranchesmontagnesch1', 'dxvv_christopheJ', 'er3z4aet1234',$pdo_options);  
	...
	/*
//	$req = $bdd->prepare('UPDATE inscription SET equipe =?,  WHERE Nom=\''.$_POST['Nom1']. '\ AND  Prenom=\''.$_POST['Prenom1']. '\'');
		// $req = $bdd->exec('UPDATE `inscription` SET`equipe`=?, WHERE Nom=\''.$_POST['Nom1']. '\ AND  Prenom=\''.$_POST['Prenom1']. '\'');
...
$req = $bdd->prepare('UPDATE inscription SET equipe = :equipe1  WHERE Nom = :nom1 AND  Prenom = :prenom1 AND course = :course1');
$req->execute(array(
	'equipe1' => $_POST['Nom_equipe'],
	'prenom1' => $_POST['Prenom1'],
	'nom1' => $_POST['Nom1'],
	'course1' => 'Course des Franches2019'
	));
...
$req->execute(array(
	'equipe1' => $_POST['Nom_equipe'],
	'prenom1' => $_POST['Prenom3'],
	'nom1' => $_POST['Nom3'],
	'course1' => 'Course des Franches2019'
	));
	
	 mail('inscription@juradefichrono.ch', "inscription équipe".$post['ID'], 	$_POST['Nom_equipe']."\n".$_POST['email']);
...
Votre inscription a été validée, veuillez vérifier si votre équipe  est inscrite </br>dans la liste des équipes inscrits sur le lien suivant: 
<a href='liste.php'>Liste des équipes</a></br>Si ce n'est pas le cas, envoyez un mail à l'adresse suivante: marcbaume12@gmail.com
</br>
</br>
<?php
}
